 		...
 		if ($className == 'LinkListLinkPage') {
			if ($eventName == 'readData') {
				$sql = "SELECT		linkList_link.*, user_renommee.evaluatorID AS rated,
						...
						"(positiveRenommeeRating - negativeRenommeeRating) AS objectRenommee" : "")."
			...
			if ($eventName == 'readData') {
				if (USER_RENOMMEE_SHOW_RENOMMEE) {
					...
					$eventObj->linkList->sqlSelects .= "(positiveRenommeeRating - negativeRenommeeRating) AS objectRenommee";
				}
