...
require_once(WCF_DIR.'lib/data/user/renommee/RenommeeEditor.class.php');
...
class LinkListLinkCommentRenommeeCronjob implements Cronjob {
	...
	public function execute($data) {
		...
		if (!empty($userIDs)) {
			$sql = "SELECT renommeeID FROM	wcf".WCF_N."_user_renommee
				WHERE		objectID NOT IN (SELECT commentID
								FROM	wcf".WCF_N."_linklist_link_comment)
						AND	objectType = 'linkListLinkComment'";
			WCF::getDB()->sendQuery($sql);
			
			$renommeeIDArray = array();
            while ($row = WCF::getDB()->fetchArray($result)) {
                $renommeeIDArray[] = $row['renommeeID'];
            }

            if (count ($renommeeIDArray)) {
                RenommeeEditor::deleteAll(implode(',', $renommeeIDArray));
            }
			// get users
