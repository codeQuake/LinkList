...
{elseif LINKLIST_NEWESTCOMMENTS_TYPE == 2}
						...
						{foreach from=$comments item=comment}
							<tr class="{cycle name='className'} normalFont">				
								<td class="columnTopXComments" style="width:25%;">
									<img src="{icon}linkListLinkS.png{/icon}" alt="" />
									<a href="index.php?page=LinkListLink&amp;linkID={@$comment->linkID}{@SID_ARG_2ND}"> <span>{$comment->link->subject|truncate:35:"..."}</span></a> &raquo;
								</td>                        
...
{else}
				<div class="border newestComments">
                    <div class="containerHead">
                         <div class="containerIcon">
                            <a href="javascript: void(0)" onclick="openList('linkListShowNewestComments', { save:true })">
                                <img src="{icon}minusS.png{/icon}" id="linkListShowNewestCommentsImage" alt="" />
                   ...
                   <ul class="dataList">
                    {cycle values='container-1,container-2' name='className' print=false advance=false}
                    {if !$comments|count}
					<p>{lang}{lang}wcf.linkList.newestComments.noComments{/lang}{/lang}<p>
					{else}
                            {foreach from=$comments item=comment}
                                ...
                                <div class="containerContent">
                                    ...
                                    {if $comment->userID}
											...
											{else}
												{$comment->username}
											{/if} 
											<span class="light smallFont">({@$comment->time|shorttime})</span>
                                    </p>
                                </div>
